classdef UNMTrafficSignal_Block < matlab.System
    ...
    methods (Access = protected)
        ...
        function [state, tLeft, distToStop] = stepImpl(obj,egoPose,pathAction,signalRuntime)
            ...
            %%From ALL traffic lights in scene, find the one with the specified SignalID


            if isempty(signalIdx)
                fprintf('WARNING: Signal ID %d not found! Available IDs: ', obj.SignalID);
                disp(allSignalActorID)  % prints all available signal IDs
                return;
            end
           ...
           %% Find the distance to stop line
            %Get ego current lane position & Check whether it is on a lane 
            egoLaneLoc = obj.EgoActorSim.getAttribute("LaneLocation");
            isOnLane = egoLaneLoc ~= 0;  % 0 means not on lane 
            
            juncIdx = obj.MainJunctionIdx;

            %Get the junction boundary
            region = obj.RRHDMap.Junctions(juncIdx).Geometry.Polygons.ExteriorRing;
            juncCenter = [mean(region(:,1)), mean(region(:,2))];

            % Get position correctly from 4x4 pose matrix
            distToJunction = norm(egoPose(1:2,4)' - juncCenter);
 
            if distToJunction < 100 && isOnLane   % On lane and approaching the main junction
                ...
                % Find the index on which the ego is located / closest point
                % on path to ego vehicle
